...
function generateGapAdd2Sub2(rnd: () => number, idx: number, d: Difficulty): { gap: GapConfig; planks: Plank[] } {
  const p = paramsForDifficulty(d)
  const t = randInt(rnd, p.targetRange[0], p.targetRange[1])
  const c = randInt(rnd, p.allowZero ? 0 : 1, Math.min(p.plankRange[1], Math.floor((p.plankRange[1] + t) / 2)))
  const d2 = randInt(rnd, p.allowZero ? 0 : 1, Math.min(p.plankRange[1], Math.floor((p.plankRange[1] + t - c) / 2)))
  const sumNeeded = t + c + d2
  const a = randInt(rnd, p.allowZero ? 0 : 1, Math.min(p.plankRange[1], sumNeeded))
  const b = sumNeeded - a
  if (b < (p.allowZero ? 0 : 1) || b > p.plankRange[1]) {
    return generateGapAdd2Sub2(rnd, idx, d)
  }
  ...
  while (planks.length < base.length + p.decoyCount + 4) {
    const v = randInt(rnd, p.plankRange[0], p.plankRange[1])
    if (!set.has(v)) {
      set.add(v)
      planks.push(makePlank(gap.id, v))
